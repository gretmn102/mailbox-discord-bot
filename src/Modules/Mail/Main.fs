    ...
    let help postmanType =
        ...
        | SantaClaus ->
            [
                ...
                sprintf "• `%s` — сохранить письмо и вернуться в главное меню, чтобы написать кучу новых поздравлений, либо же отредактировать существующие!" CommandNames.returnToMails
            ]
            |> String.concat "\n"
        | Valentine ->
            [
                ...
                sprintf "• `%s <произвольный_набор_символов>` — указывает содержимое сообщения" CommandNames.description
                sprintf "• `%s [<ссылка на картинку>]` — добавляет внизу картинку. Чтобы убрать, напишите команду без аргументов" CommandNames.image
                sprintf "• `%s` — сохранить валентинку и вернуться в главное меню, чтобы написать кучу новых валентинок, либо же отредактировать существующие!" CommandNames.returnToMails
            ]
            |> String.concat "\n"
